...
## Tools Setup / ゲーム更新時のクリーン展開チェックリスト（git 管理外）

`tools/` ディレクトリは未追跡。ゲームの `.pck` からアセットを展開する。**ゲーム更新のたびに
下記を 1〜5 の順で全部回す**（`tools/extracted` を作り直すと、後段の生成物もすべて消えるため）。

```powershell
# 1. PCKExplorer でゲームアセットを展開（約 1.8 GB）
...
# 2. メタデータ抽出
dotnet run --project card-type-extractor

# 3. .ctex → PNG（無引数＝カードポートレート）
dotnet run --project ctex-to-png
dotnet run --project ctex-to-png -- relics
dotnet run --project ctex-to-png -- events
dotnet run --project ctex-to-png -- ancients
dotnet run --project ctex-to-png -- potions

# 4. card_images.json は 3 の出力をスキャンして作るので extractor をもう一度回す
dotnet run --project card-type-extractor

# 5. モンスター画像（Spine → PNG/GIF）→ tools/extracted/images/monsters/
dotnet run --project ctex-to-png -- monsters
```
...
画像は BC7 圧縮テクスチャ（`.ctex`）で、ctex-to-png が PNG 変換してキャッシュする。

### 手順 5 の注意（過去に踏んだ罠）

- `images/monsters/{id}.png` `{id}.gif` は **`ctex-to-png -- monsters` の生成物**（`MonsterGifGenerator`。
  旧 StS2SiteBuilder の `GenerateMonsterGifs` を移設）。出力先が `tools/extracted/images/monsters/`
  ＝ 展開先の中なので、**クリーン展開で必ず消える**。展開後は必ず再実行すること。
- `AssetLocator` は `tools/extracted` が見つからないと**配布用アセットキャッシュ
  （`%LocalAppData%\SpireScope\assets\v*`）へ黙ってフォールバックする**（v0.109.0 で site3 の
  モンスター画像が全欠落した事故の原因）。対策として `monsters` サブコマンドは
  `AssetLocator.HasDevExtracted()`（walk-up のみ）でガードし、リポジトリの `tools/extracted` が
  無ければ hard fail する。ログ末尾の `Generated monsters ... -> <パス>` が本リポジトリ配下かで検証できる。
- 生成された PNG は **site3（`slaythespire2.lichtenlab.com`）が read-only マウントで直接参照**
  している（`site3/docker-compose.yml` の `images/monsters:/var/www/html/img/monsters:ro`）。
  site3 側の `scripts/import.php` が取り込み時に欠落を検出して警告する。
...
| `StS2Shared.Spine` | Spine 描画の共有ライブラリ（`SpineLoader`/`SpineRenderer`/`CreatureVisual`。`IAssetSource` でディスク/pck 両対応、`MonsterPngRenderer`）。ctex-to-png（monsters）と配布セットアップが共用 |
...
| `StS2SiteBuilder` | 成果物インベントリ CLI（ソリューション生成物を実スキャンして `out/artifacts.html` を出力） |
| `card-type-extractor` | ゲーム DLL の IL を解析してカードメタデータ JSON を生成するCLIツール |
| `ctex-to-png` | `.ctex`→PNG 変換 CLI（relics/events/ancients/potions/monsters ほか、extract-pck サブコマンドあり） |
...
- `character_colors.json` — キャラクター（`IRONCLAD` 等）の色。各 `CharacterModel`（`Models.Characters`）の Color プロパティと
  ...
  （`mapDrawingColor`/`dialogueColor`/`energyOutlineColor`/`targetingLineColor`）を持つ。`CharacterColorService` が読む
...
- `card_images.json` — カード ID → 画像のソース相対パス（`card_portraits_png/` 基準、例 `silent/abrasive.png`）。
  extractor が実ファイルをスキャンして生成。`Services/CardImageService.cs` で参照し、画像解決を一元化。
...
| `CharacterMechanics.cs` | キャラクター × メカニクスのフィルタ定義（`Func<string, bool>` の配列）。Toys のキャラクター概観が参照 |
...
### ctex-to-png — monsters（モンスター GIF/PNG 生成）

`dotnet run --project ctex-to-png -- monsters` で実行（`MonsterGifGenerator`。
旧 StS2SiteBuilder の `GenerateMonsterGifs` を移設したもの）。

- `monster_names.json` の各モンスター **ID 単位**で
  ...
  - `crusher`/`rocket` など `visible=false` の内部サブモンスターは画像を生成しない（正当な無画像）。
...
`Services/EncounterActService.cs`（`EventActService` のエンカウンター版）で読む。
モンスターは**モデル粒度**の ID 空間（例 `bowlbug_rock`）で、`monster_names` と `encounter_monsters` で共通。
画像は ID をそのまま使うため、Spine フォルダがある ID のみ画像化される。
Spine レンダリングには `StS2Shared.Spine`（`SpineLoader`/`SpineRenderer`/`CreatureVisual`、
`DiskAssetSource` 経由）と `SpineRuntime` プロジェクト（spine-csharp の pure C# ランタイム）を使用する。

### StS2SiteBuilder — 成果物インベントリ CLI

`dotnet run --project StS2SiteBuilder` で `StS2SiteBuilder/out/artifacts.html` を出力する
（`--out <パス>` で変更可。gitignore 対象）。かつては静的サイトジェネレータだったが、
生成サイトの廃止に伴い、ソリューション成果物の全体像を把握するレポートツールへ転換した（サイト生成コードは git 履歴参照）。

- **依存プロジェクトなし**（`AssetLocator` も不使用）。自分の位置から `repoRoot` を計算する
  純粋なディスクスキャナで、`%LocalAppData%` フォールバック罠を構造的に回避する。
- `ArtifactCatalog.cs` — 成果物グループの静的メタデータ（生成元・コマンド・出力先・参照元）。
  **生成コマンドや参照元が変わったらここも更新する。**
- `ArtifactInventory.cs` — 実ファイルスキャン：`Resources/v*/` のバージョン × ファイル presence
  マトリクス（最新版はサイズ・件数つき）、手動管理 JSON、`tools/extracted/images/` の実数と
  `*_images.json` / `monster_names.json` 由来の期待数・欠落一覧。`tools/extracted` 不在時は
  バナーを出して画像セクションを優雅にスキップする。
