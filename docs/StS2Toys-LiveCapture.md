# StS2Toys ライブキャプチャ表示機能 仕様書

最終更新: 2026-06-23 / 対象バージョン: ゲーム v0.107.1 / ステータス: 設計確定（実装は別タスク）

## 1. 目的・背景

StS2Toys（セーブデータビューア）に、**プレイ中のゲーム画面をキャプチャして関連情報を表示するフォーム**を追加する。
...
`StS2Capture`(exe) と `StS2Toys` の双方がこれを参照する。**StS2Toys の TFM を `net10.0-windows` →
...
| `StS2Toys` | `net10.0-windows10.0.19041.0`（引上げ） | `Core` ＋ `StS2Shared` を参照。新フォームを追加 |
...
補助変換（StS2SiteBuilder の `RawId`/`GetCardDir` と同一規則）：
...
既存 `WindowSettingsService` / `AppSettings`（StS2Toys）に項目を追加する。
...
1. `StS2Capture.Core` 抽出 ＋ 両プロジェクト参照、StS2Toys TFM 引上げ、ソリューション全体ビルド。
...
- 非回帰：StS2Toys 既存フォーム、StS2Capture 単体動作。
...
- StS2SiteBuilder：`SiteBuilderCore.cs`（`RawId`/`GetCardDir`/`MecFileName`、各 `Build*Page` と href 生成、`NodeBadge`）
- StS2Toys：`Form1.cs`（子フォーム定石・`DisplayData`）/ `Services/AppLanguage.cs` / `Services/WindowSettingsService.cs`
